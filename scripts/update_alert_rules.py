...
def process_rule_file(contents, destination_file: Path, source_url: str):
    ...
    This function also filters out unwanted records and rules, and writes the
    processed rules to a new file.
    ...
    for group in alert_rules["groups"]:
        group["rules"] = [
            ...
            and rule.get("alert") and rule["alert"] in REQUIRED_RULES
        ]

    # Filter out groups that have no rules after filtering
    alert_rules["groups"]= [
        group
        for group in alert_rules["groups"]
        if group.get("rules")
